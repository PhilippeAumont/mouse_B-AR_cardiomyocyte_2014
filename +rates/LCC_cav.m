function dS_LCC = LCC_cav(S, C_cav, Ca, V, p)
  ...
  c1_c1p = rates.P(c1,C,I_CaL_cav_tot);
  c1p_c1 = rates.DP(c1p,PP,I_CaL_cav_tot)*ap^3*k_cop/(a^3*k_co);
  c2_c2p = rates.P(c2,C,I_CaL_cav_tot);
  c2p_c2 = rates.DP(c2p,PP,I_CaL_cav_tot)*ap^2*k_cop/(a^2*k_co);
  c3_c3p = rates.P(c3,C,I_CaL_cav_tot);
  c3p_c3 = rates.DP(c3p,PP,I_CaL_cav_tot)*ap*k_cop/(a*k_co);
  c4_c4p = rates.P(c4,C,I_CaL_cav_tot);
  c4p_c4 = rates.DP(c4p,PP,I_CaL_cav_tot)*k_cop/k_co;
  cp_cpp = rates.P(cp,C,I_CaL_cav_tot);
  cpp_cp = rates.DP(cpp,PP,I_CaL_cav_tot)*a*k_cop/(ap*k_co);
  o_op = rates.P(o,C,I_CaL_cav_tot);
  op_o = rates.DP(op,PP,I_CaL_cav_tot)*a/ap;
  i1_i1p = rates.P(i1,C,I_CaL_cav_tot);
  i1p_i1 = rates.DP(i1p,PP,I_CaL_cav_tot)*a/ap;
  i2_i2p = rates.P(i2,C,I_CaL_cav_tot);
  i2p_i2 = rates.DP(i2p,PP,I_CaL_cav_tot);
  i3_i3p = rates.P(i3,C,I_CaL_cav_tot);
  i3p_i3 = rates.DP(i3p,PP,I_CaL_cav_tot);
  ...
  C3 =  [0,0,3*a,-(2*a+c3_c3p+a*B),3*B,0,0,0,0,0,0,0,c3p_c3,0,0,0,0,0];
  ...
  Op =  [o_op,0,0,0,0,0,0,0,0,-(y+op_o*a/ap+0.001*K_pcf+k_oc),0,0,0,0,k_cop,K_pcb,0.001*ap,0];
  ...
  I3p = [0,0,0,0,0,0,0,0,i3_i3p,0,0,0,0,y*K_pcf*k_cop/k_oc,0,0.001*K_pcf,y,-(4*B*K_pcb+i3p_i3+K_pcb)];
