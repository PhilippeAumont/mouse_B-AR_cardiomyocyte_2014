...
%Membrane Potential
psi     = (p.F/p.C_m/1e6)*p.V_Lys*(H + K + Na - Cl + 2*Ca_T - p.B);
...
Cl_C0   = Cl_C*exp(p.psi_out/p.RTF);
...
%Passive flux
J_H    = p.P_H*p.S*(10^(-pH_C0)*exp(-psi/p.RTF)-10^(-pH_L0))*gg*p.NA;
J_K    = p.P_K*p.S*(K_C0*exp(-psi/p.RTF)-K_L0)*gg*p.NA;
J_Na   = p.P_Na*p.S*(Na_C0*exp(-psi/p.RTF)-Na_L0)*gg*p.NA;
J_Cl_unc   = p.P_Cl*p.S*(Cl_C0-Cl_L0*exp(-psi/p.RTF))*gg*p.NA;
J_Ca   = p.P_Ca*p.S*(Ca_F_C0*exp(-2*psi/p.RTF)-Ca_F_L0)*gg_Ca*p.NA;

%TRPML1 channel
y = 0.5 - 0.5*tanh(psi + 40);
P_trpml1 = p.p_trpml1*(y*abs(psi) + (1-y)*(abs(psi + 40)^3)/(pH_L0^p.q));
J_Ca_trpml1 = P_trpml1*p.S*(Ca_F_C0*exp(-2*psi/p.RTF)-Ca_F_L0)*gg_Ca*p.NA;

%Time Dependent Quantities

dNHdt   = J_H + (J_VATPASE) - (p.CLC_H*J_CLC) - (p.CAX_H*J_CAX);

dpHdt   = (-dNHdt/p.V_Lys/p.NA)/beta_pH;
